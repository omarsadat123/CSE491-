...
int main(int argc, char** argv) {
    ...
    if (act == "p") {                           // pre-process
        ...
        clean_edges(src_filename.c_str(), clean_filename.c_str()); //to remove duplicates and self-loops
...
//        if (suffix != ".index") exit(0);
        ...
        // Post-processing to remove duplicate cliques
        
        // 1. Sort the vertices within each clique
        for(auto& clique : stored_cliques) {
            std::sort(clique.begin(), clique.end());
        }

        // 2. Sort the entire list of cliques to group duplicates together
        std::sort(stored_cliques.begin(), stored_cliques.end());

        // 3. Erase adjacent duplicates
        stored_cliques.erase(
            std::unique(stored_cliques.begin(), stored_cliques.end()),
            stored_cliques.end()
        );
        // --- End of Modification ---

        printf("Total cliques stored: %zu\n", stored_cliques.size());

        // Print first few cliques as example
        printf("First 10 cliques found:\n");
        for (int i = 0; i < std::min(10, (int)stored_cliques.size()); i++) {
            printf("Clique %d: ", i+1);
            for (int v : stored_cliques[i]) {
                printf("%d ", v);
            }
            printf("\n");
        }
