...
public:
    PseudoCliqueEnumerator(Graph& graph, float theta = 1.0, int min_size = 1, int max_size = -1)
        ...
        for (const auto& v : neighbors_and_P[0]) {
            //int degree_np = graph.adj_map[v].size();
...
void PseudoCliqueEnumerator::add_to_inside_P(int v) {
    ...
    if (total_nodes_in_P >= min_size) {
        // Debug output to understand pseudo-clique detection
        //double density = (2.0 * total_edges_in_P) / (total_nodes_in_P * (total_nodes_in_P - 1));
        //if (density >= theta) {
          //  std::cout << "Found pseudo-clique of size " << total_nodes_in_P 
            //         << " with density " << density 
              //       << " (edges: " << total_edges_in_P << ")" << std::endl;
...
int main(int argc, char* argv[]) {
    ...
    // Order Bound Pruning Technique
    std::vector<int> core_numbers = compute_core_numbers_bin(graph);
    int degeneracy = 0;
    if (!core_numbers.empty()) {
        degeneracy = *std::max_element(core_numbers.begin(), core_numbers.end());
    }
    std::cout << "Theta: " << theta << "\n";
    std::cout << "Degeneracy (xi_G): " << degeneracy << std::endl;

    // Calculate bounds using input theta value
    double bound1 = std::ceil(2.0 * degeneracy / theta);  // Use ceil instead of floor
    double bound2 = std::floor(1.0 / (1.0 - (static_cast<double>(degeneracy) / ((degeneracy + 1) * theta))));
    int mu = static_cast<int>(std::min(bound1, bound2) * 1.2);  // Add 20% tolerance
    std::cout << "Order bound 1 (Corollary 1): " << bound1 << std::endl;
    std::cout << "Order bound 2 (Corollary 2): " << bound2 << std::endl;
    std::cout << "Final order bound (mu): " << mu << std::endl;

    if (minimum > mu) {
        std::cout << "No (l, theta)-pseudo-clique exists: l = " << minimum << ", mu = " << mu << std::endl;
        return 0;
    }
    if (maximum > mu) {
        maximum = mu;
    ...
    bool found = false;
    for (size_t size = 1 ; size < pseudo_clique_counts.size(); ++size) {
        if (pseudo_clique_counts[size] == 0) {
            break;
        }
        
        std::cout << "Size " << size + minimum << ": " << pseudo_clique_counts[size] << "\n";
        found = true;
        
    }
    if (!found) {
        std::cout << "None found for the given minimum size and parameters." << std::endl;
    }
